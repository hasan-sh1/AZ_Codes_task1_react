...
// form post
const FormPost = () => {
  return <form action="#" className="comment-area-box">
...
const PostComment = ({
  ...
  return <>
      <div className={classnames("d-flex", "align-items-start", containerClass)}>
        ...
        <div className="w-100">
          ...
          <p className="mb-0" dangerouslySetInnerHTML={createMarkup(comment.content)}></p>

          {isActionAllowed && <Link to="#" className="text-muted font-13 d-inline-block mt-2">
              <i className="mdi mdi-reply me-1"></i> Reply
            </Link>}

          {comment.replies && <>
              {(comment.replies || []).map((reply, index) => {
            return <PostComment key={index} comment={reply} isActionAllowed={false} toggleLikeOnComment={toggleLikeOnComment} containerClass="mt-3" />;
          })}
            </>}
        </div>
      </div>
  ...
  return <Card>
      <Card.Body>
        <div className="d-flex align-items-start">
          <img className="me-2 avatar-sm rounded-circle" src={post.author.avatar} alt="" height="32" />
          <div className="w-100">
            <Dropdown className="float-end text-muted" align="end">
              <Dropdown.Toggle as="a" className="text-muted font-18 cursor-pointer">
        ...
        <div className="mt-2">
          <Link to="#" className="btn btn-sm btn-link text-muted ps-0" onClick={() => toggleLike(post)}>
            <i className={classnames("mdi", "text-danger", post.isLiked ? "mdi-heart" : "mdi-heart-outline")}></i>{" "}
          ...
          {post.totalView && <Link to="#" className="btn btn-sm btn-link text-muted">
        ...
        {post.comments && <div className="post-user-comment-box mt-2">
            {(post.comments || []).map((comment, index) => {
          return <PostComment key={index} comment={comment} isActionAllowed={true} toggleLikeOnComment={toggleLikeOnComment}></PostComment>;
        })}
            {user && <div className="d-flex align-items-start mt-2">
                <Link className="pe-2" to="#">
                  <img src={user.avatar} height="31" className="rounded-circle" alt="" />
                </Link>
                <div className="w-100">
        ...
        {user && !post.comments && <div className="d-flex align-items-start mt-2">
...
const Feeds = () => {
  ...
  return <>
      ...
      return <Post post={post} key={index} user={user} toggleLike={toggleLike} toggleLikeOnComment={toggleLikeOnComment} />;
    })}
