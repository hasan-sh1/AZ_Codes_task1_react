...
const ColoredCards = () => {
  const colors = ["primary", "success", "info", "warning", "danger", "blue", "pink", "dark"];
  return <>
      <Col md={4}>
...
const BorderdCards = () => {
  ...
  return <>
      ...
      {(colors || []).map((item, index) => {
      return <Col md={4} key={index}>
            <Card className={classNames("border", [`border-${item.variant}`], "mb-3")}>
              <Card.Header>Header</Card.Header>
              <Card.Body>
                <Card.Title as="h4" className={classNames([`text-${item.variant}`])}>
                  {item.name} card title
                </Card.Title>
                <Card.Text>
                  Some quick example text to build on the card title and make up
